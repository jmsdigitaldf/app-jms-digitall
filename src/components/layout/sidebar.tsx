...
import { LayoutDashboard, Users, Ticket, FileText, Settings, Wrench } from 'lucide-react';
      ...
      <nav className="flex-1 space-y-1 px-3 py-4 overflow-y-auto">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn('h-5 w-5 flex-shrink-0', isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-500')} />
              {item.name}
            </Link>
          );
        })}
